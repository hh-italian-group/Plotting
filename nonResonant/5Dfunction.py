#!/usr/bin/env python
# RUN: python 5Dfunction.py --LHC 7 --kl 1 --kt 1 --c2 0 --cg 0 --c2g 0
...
parser.add_option("--LHC", type="int", dest="num", help="LHC CM energy in TeV")
...
topMass =0.1
#XS = float(format(f(kl,kt,c2,cg,c2g)*xs,'.2f'))
XS = round(f(kl,kt,c2,cg,c2g)*xs,2) 

#print(f(1,1,0,0,0)*xs)
# by now we ignore errors in the RHH errors
#print "XS =  %f fb" % (round(f(kl,kt,c2,cg,c2g)*xs,2))
print "%f" % (round(f(kl,kt,c2,cg,c2g)*xs,2))

#print "Uncertainties (fb): scale (+ %f, %f), PDF +- %f, alphaS +- %f, top mass +- %f, RHH +- 0 " % ( round(f(kl,kt,c2,cg,c2g)*scalep,2), round(f(kl,kt,c2,cg,c2g)*scalem,2), round(f(kl,kt,c2,cg,c2g)*PDF,2), round(f(kl,kt,c2,cg,c2g)*alphas,2), round(f(kl,kt,c2,cg,c2g)*topMass*xs,2) )
